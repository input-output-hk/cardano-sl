...
import           Pos.Core.Configuration        (HasConfiguration, slotDuration)
...
sendToAllGenesis sendActions (SendToAllGenesisParams duration conc delay_ sendMode tpsSentFile) = do
    ...
    let actualSlotDuration = fromIntegral (toMicroseconds slotDuration) `div` 1000000 :: Int
    ...
    Mockable.bracket (openFile tpsSentFile WriteMode) (liftIO . hClose) $ \h -> do
        ...
        liftIO . T.hPutStrLn h $ T.intercalate "," [ "slotDuration=" <> show actualSlotDuration
