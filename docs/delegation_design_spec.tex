...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[ align = center
    ...
    %%
    \draw[->] (owner) -- node[above] {owns}
              (addr);
    \draw[->] (addr) -- node[left] {contains a}
              (pkey);
    \draw[->] (addr) -- node [above, rotate=-40]{contains a} (skey);
    \draw[->] (owner) -- node[left] {owns} (skey);
    \draw[->] (owner) -- node[left] {owns} (racc);
    \draw[->] (pkey) -- node[anchor=south] {is associated\\ with a} (skey);
    \draw[->] (skey) -- node[anchor=south] {is associated\\ with a} (racc);
    \draw[->] (racc) -- node[above, rotate=90] {collects} (rewards);
    \draw[->] (pkey) -- node[anchor=west] {has} (funds);
    \draw[->] (skey) -- node[anchor=west] {has} (srights);
    \draw[->] (srights) -- node[anchor=north] {associated with} (funds);
    \draw[->] (pos) edge[bend left] node[left] {pays to} (racc);
    \draw[->] (srights) -- node[anchor=east] {delegated to} (spool);
    \draw[->] (spool) -- node[anchor=south] {participates in} (pos);
  \end{tikzpicture}
...
\begin{figure*}[h]
  ...
  \draw[-] (utxo) -- node[pos=0.1] {$n$}
                     node[pos=0.85] {$1$}
           (addr);
  \draw[-] (addr) -- node[pos=0.1] {$n$}
                     node[pos=0.85] {$0 \ldots 1$}
           (pkey);
  \draw[-] (skey) -- node[pos=0.1] {$n$}
                     node[pos=0.85] {$0 \ldots 1$}
           (spool);
  \draw[-] (skey) -- node[pos=0.1] {$1$}
                     node[pos=0.9] {$1$}
           (racc);           
  % Horizontal lines.
