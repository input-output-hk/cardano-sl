...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[ align = center
    ...
    \draw[->] (spool) -- node[anchor=south, rotate=50] {pays to} (racc);
...
\item There can be many UTxO entries (funds) for a single address. However a
  UTxO entry refers to an input and an output address, unless it is a special
  kind of transaction (for instance those that forge value, and have no
  corresponding input).
