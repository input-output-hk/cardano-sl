...
delegation, and their relationships.

\begin{figure}[h]
  \centering
  \begin{tikzpicture}[ align = center
                     , node distance = 5em and 8em
                     , text width = 5em
    ...
    %%
    \node (owner) {Owner};
    \node (skey) [below = of owner] {Staking key};
    \node (pkey) [left = of skey] {Payment key  (address?)};
    \node (racc)  [right = of skey] {Rewards account (address?)};
    \node (rewards) [below = of racc] {Rewards};
    \node (funds) [below = of pkey] {Funds};
    \node (srights) [below = of skey] {Staking rights};
    \node (spool) [below = of srights] {Stake pool};
    \node (pos) [right = of spool] {PoS};    
  
    \tikzset{every node/.style={align=center, text width=4em, text=brown}}
    %% 
    %% Relationship between nodes (arrows)
    %%
    \draw[->] (owner) -- node[anchor=east] {owns} (pkey);
    \draw[->] (owner) -- node[anchor=east] {owns} (skey);
    \draw[->] (owner) -- node[anchor=east] {owns} (racc);    
    \draw[->] (pkey) -- node[anchor=south] {has a} (skey);
    \draw[->] (skey) -- node[anchor=south] {has a} (racc);
    \draw[->] (racc) -- node[anchor=west] {collects} (rewards);
    ...
    \draw[->] (spool) -- node[anchor=south, rotate=45] {pays to} (racc);
