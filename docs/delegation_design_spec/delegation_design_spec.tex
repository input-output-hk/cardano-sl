...
\todo{This section needs to be updated. In particular, with the stake
  account mechanism for sharing rewards, we no longer need to know
  each address that belongs to a staking key, only the total stake for
  each staking key.}
...
\subsection{Slot leader schedule}
\label{slot-leader-schedule}

The process of leader election has to be modified to take delegation
into account.

While adding blocks to their chain, nodes will keep track of the stake
that each registered stake key controls, and of all valid
(heavyweight) delegation certificates.

When the schedule for the next epoch has to be constructed, the nodes
will compute the stake per registered stake pool. This is called the
\emph{Stake Distribution}. For each slot, a leader is determined by
pseudo-randomly sampling the stake pools, weighted by the stake that
each pool controls.

In order to ensure consensus on the leader schedule between all nodes,
the stake distribution is calculated from the state of the chain $2k$
slots before the end of the epoch.\footnote{This can be done
...
\todo{This section needs to be updated, to reflect the new plan for
  handover (BFT and the slider to determine the fraction of blocks
  created by the bootstrap nodes).}
...
\todo{This section needs to be updated. In particular, the mechanism
  for distributing rewards has changed to use stake accounts instead
  of automatic UTxO updates.}
