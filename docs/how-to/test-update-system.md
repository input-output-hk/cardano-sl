...
Launch `cardano-rubbish` with CLI:

```
stack exec -- cardano-rubbish --system-start 0 --log-config log-config-prod.yaml --logs-prefix "logs/abc4" --db-path db-abc4 --peer {relayHost}:3000 repl
```
