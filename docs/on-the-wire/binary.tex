...
\item[Variants] We denote variants as:
  ...
  \end{grammar}
\item[Optional values] We denote an optional value of type $A$ as a variant
  $\emptyset \ |A$
\end{description}
...
introduce the block version in this section, since it is primarily concerned
with updates.
...
\begin{figure}[H]
  \begin{grammar}
    ...
    <$\upd$> = \{
    \fld proposal :: $\emptyset | \updprop$
    \fld votes :: $\lists{\verb|UpdateVote|}$
    \}
  \end{grammar}
  \caption{Updates}
  \label{fig:updtypes}
