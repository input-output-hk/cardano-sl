...
\item[Chain] A chain is a sequence of blocks $b₁,…,b_n$ such that the
  predecessor of $b₁$ is the genesis block $b₀$, and for every $i$, the
...
\newcommand{\stakerepartitionformula}[4]{∑_{#2∈#3} \left(
    ...
    total value of $o$.  \improvement{The treatment of remainder as
      specified here is only valid for the bootstrap era.}
  \end{itemize}
...
\item[Young fork] Let $b₁,…,b_i$ the longest prefix of $b₁,…,b_n$
  ...
  ancestor is at most $k$ block deep).\improvement{This looks
    different than what is done in the actual Cardano implementation:
    instead of rejecting the incoming block, it appears that the
    slogRollbackBlocks function aborts the node instead.}
