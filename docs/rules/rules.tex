...
\item[Lists] Given a set $A$, we write $\lists{A}$ for the set of
  ...
  $l,r$ for their concatenation. We write $\emptylist$ for the empty
  list.For a list $l∈\lists{A}$, we write $\length{l}$ for its length.
  We write $\pos{i}{l}$ for the $i$-th element of the
  list (the head of the list is $\pos{1}{l}$).
...
We can then define the slot leader function as\improvement{This relies
  ...
  specified.}\improvement{The formula works for $u_n ⩾ 1$ but $u_n$ can be
