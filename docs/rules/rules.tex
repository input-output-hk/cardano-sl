...
\item[Transactions] Each transaction in the block's transactions is
  ...
  \item[Stateful] Stateful transaction validation rules are
    parametrised by a list of transaction $τ_1,…,τ_m$ which we will
    ...
    \begin{description}
    \item[Unspent inputs] \improvement{Add some spacing for
        readability} Each input of the transaction $τ$ is
      unspent. That is, for each $i ∈ \inputs{τ}$,
      $\overline{τ} = \overline{τ₀},\intrans{i}, \overline{τ_1}$,
      $\outputs{\intrans{i}}=t₁,…t_k$, $\inindex{i} ⩽ k$, and for each
      $t ∈ \overline{τ₁}$, $i ∉ \inputs{t}$.

      In these condition, we can extend the function $\recipient{⋅}$
    ...
    \item[Conservation of value] The total amount of inputs must equal
      ...
      $$
      ∑_{i∈\inputs{τ}} \amount{i} ⩽ ∑_{o∈\outputs{τ}} \amount{o}
      $$
