...
Let\improvement{define the remainder notation}{}
$n₀ = \min_{n>0} \left(\rem{2^{kB}}{N} ⩽ u_n < 2^{kB}\right)$.
...
$\{ p∈ℕ | \rem{2^{kB}}{N} ⩽ u_n < 2^{kB} \}$ has $aN$ elements for
some $a$. From which we can deduce that $f(N)$ is surjective on its
