...
\newcommand{\txids}{\idsof{\mathcal{T}}}
...
\item[Transaction] Transactions have a system of identifiers
  ...
  A transaction consists of\improvement{Consider using record notation
  ...
  \item[Outputs] A non-empty list of pairs in $\agentids ×
    ℕ$.\improvement{This only embodies pay-to-public-key schemes. For
      ...
      refinements.}\improvement{Explain that the natural number is an
      amount of currency.}
...
\item[Transactions]\improvement{Explain how to validate each
    transaction on top of the others}\hfill
  \begin{description}
  \item[Stateless]\hfill
    ...
    \end{description}
  \item[Stateful]\improvement{Make this section more precise}\hfill
    \begin{description}
    \item[Unspent inputs] Each input of the transaction is unspent.
    \item[Conservation of value] The total amount of input must equal
      the total amount of outputs.\critical{This ignores transaction
        fees and asset creation. How are they represented, what are
        their rules?}
    \item[Authorised] The list of witnesses has the same length as the
      list of input, and for each input, the corresponding witness is
      the agent id of the input.
    \end{description}
  \end{description}
\end{description}
