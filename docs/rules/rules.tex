...
\item[Current slot] The current slot is a parameter of every verification
  ...
  at slot $i$.\unsure{This sentence is unclear to me. As verification hasn't
    been discussed yet, maybe just leave it out?}
...
\item[Transaction] Transactions have a system of identifiers
  ...
  We assume a function $\minimumfee{⋅} ∈ \txs ⟶ ℕ$,
...
\item[Richmen] We assume a finite set $\richmen ⊆ \agentids$. These
  so-called richmen receive all the stake during the bootstrap era.
...
\item[Transactions] Each transaction in the block's transactions is
  ...
  \item[Delegation certificates] Let
    $\overline{c}=\delegationcertificates{b}$ be the list of
    delegation certificates of $b$. Then for any two
    $c₁,c₂∈\overline{c}$, it must hold that
    $\delegator{c₁}≠\delegator{c₂}$.

    Note that this prevents any agent which has issued a delegation
    certificate to another party to issue any block: they \emph{must}
    be signed by the delegatee (or, transitively, by the agent
    authorised to sign instead of the delegatee). That is, until the
    agent issues a new delegation certificate to itself, which, in
    effect, revokes any previous delegation certificate. See also the
    definition of $\transitivefix{g}$ in
  ...
  \item[Bootstrap era]\info{These rules are only used during the
    ...
    \begin{description}
    \item[No stake for poormen] \improvement{Placeholder: will
        describe the rule by which all stake must be delegated to the richmen}
    \end{description}
...
\item[Stake distribution] \info{The stake distribution at any given
    point, as well as at the last epoch}Let us define
  ...
  \item Inductively (note that we use the validity hypothesis in order
    to define $\recipient{i}$ and $\amount{i}$):\improvement{Ignores
      stake delegation}
    \begin{align*}
      \currentdistribution{τ₁,…,τ_n, τ_{n+1}}(a)
      & = &\currentdistribution{τ₁,…τ_n}(a)\\
      & - & \left(∑_{\scriptsize \begin{array}{c} i∈\inputs{τ_{n+1}}\\\recipient{i}=a \end{array}} \amount{i} \right)\\
      & + & \left(∑_{\scriptsize \begin{array}{c} o∈\outputs{τ_{n+1}}\\ \recipient{o}=a \end{array}} \amount{o} \right)
    \end{align*}
  \end{itemize}
