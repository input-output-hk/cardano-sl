...
\item[Stake repartition] A stake repartition is a function
  ...
  $∑_{a∈\agentids} r(a) = 1$.\unsure{In the bootstrap era, there is a
    single stake repartition, so the following doesn't really matter.

    But there is a further question of what is data validation, and
    what is a block validation rule. For instance, non-empty lists in
    the spec are typed as such, so if the serialised input contains an
    empty list where a non-empty list is expected, it would fail data
    validation. In a spec for after bootstrap, it must be decided
    whether the non-negativity and sums-to-one property of stake
    repartitions are similarly performed in data validation, or as
    block validation rule. My opinion is that it would be saner, more
    modular, to do it during data validation.}
