...
\item[Transactions] Each transaction in the block's transactions is
  ...
  \item[Stateful] Stateful transaction validation rules for a transaction $τ$
    ...
    \item[Conservation of value] The total amount of inputs must not exceed
      the total amount of outputs. A minimum fee must be paid.
      $$
...
\item[Young fork] Let $b₁,…,b_i$ the longest prefix of $b₁,…,b_n$
  which is also a prefix of $\mathfrak{C}$. We must have
  $\length{\mathfrak{C}} - i ⩽ k$.
