...
\newcommand{\enterpriseid}{\mathsf{dev}_\mathsf{null}}
...
A distinguished slot $\distributionslot ∈ ℕ$ is introduced to
represent the first slot beyond the bootstrap era.

There must be a distinguished agent id to represent enterprise staking
(\emph{i.e.} opting out of staking): $\enterpriseid ∈ \agentids$.

Blocks must contain an additional field\improvement{Define pool certificates}
\begin{record}
  \item[Pool certificates] A list of pool certificates
\end{record}

Transaction validation must be modified
\begin{description}
\item[Bootstrap era] These rules apply only if the slot $s$ is such that
  $s<\distributionslot$. \emph{the bootstrap era rules are otherwise
    unchanged}.
\item[Distributed era] These rules only apply if the slot $s$ is such
  that $s > \distributionslot$.
  \begin{description}
