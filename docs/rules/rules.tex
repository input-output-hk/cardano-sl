...
\newcommand{\epochcommitments}{\mathsf{commitments}}
\newcommand{\epochopens}{\mathsf{opens}}
\newcommand{\epochrecoveries}{\mathsf{recoveries}}
...
\item[Partial functions] A partial function $f$ from $A$ to $B$ is a
  pair of a subset $U∈\powerset{A}$, called the \emph{support} of $f$
  (this overloads the meaning defined in the paragraph above), and a
  function $U⟶B$. The set of partial functions from $A$ to $B$ is
  written $A ⇀ B$. We write $f(a)$ for the image of $a∈U$.

  For two partial functions $f$ and $g$ with respective supports $U$
  and $V$, such that $∀a∈U∩V. f(a)=g(a)$, we write $f⊎g$ for the partial function
  of support $U∪V$ defined as:
  $$
  f⊎g(a) = \left\{
    \begin{array}{ll}
      f(a) & \mbox{if }a∈U\\
  ...
  We say that a partial function has finite support if its support is
  a finite subset.

  \begin{implementationhint}
    A partial function $A⇀B$ can be implemented as a function \verb+A -> Maybe B+.
    Provided that its support is decidable. Mostly we are
    interested in finite support partial function which are faithfully
