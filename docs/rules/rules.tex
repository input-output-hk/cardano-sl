...
\item[Stake distribution] Let us define
  ...
  \item Inductively, and writing
    ...
    We define
    \improvement{The
      treatment of remainder as specified here is only valid for
      the bootstrap era.}
\newcommand{\stakerepartitionformula}[4]{∑_{#2∈#3} + \left\{
            \begin{aligned}
              &⌊\stakeof{#2}{#1}⌋\\
              &\kronecker{#1}{\selectrichman{\amount{#2}}}\left(\amount{#2} - ∑_{#4∈\agentids}
            ⌊\stakeof{#2}{#4}⌋\right)
            \end{aligned}\right\}}
    \begin{align*}
      \makebox[3em][l]{$\currentdistribution{τ₁,…,τ_n, τ_{n+1}}(a) =$}\\
