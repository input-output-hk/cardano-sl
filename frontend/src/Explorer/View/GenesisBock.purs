...
import Explorer.Lenses.State (_PageNumber, currentCGenesisBlockAddresses, currentCGenesisBlockSummary, gblAddressPagination, gblAddressPaginationEditable, genesisBlockViewState, lang, viewStates)
import Explorer.State (minPagination)
import Explorer.Types.Actions (Action(..))
import Explorer.Types.State (CGenesisBlockAddresses, PageNumber(..), State)
import Explorer.View.Common (getMaxPaginationNumber, paginationView)
import Network.RemoteData (RemoteData(..))
import Pos.Explorer.Web.ClientTypes (CGenesisBlockAddress, CGenesisBlockSummary)
    ...
    S.div ! S.className "explorer-genesis"$ do
        S.div ! S.className "explorer-genesis__wrapper"
              $ S.div ! S.className "explorer-genesis__container" $ do
                    ...
                    case state ^. currentCGenesisBlockSummary of
                        NotAsked  -> emptyView ""
        ...
        S.div ! S.className "explorer-genesis__wrapper" $ do
            S.div ! S.className "explorer-genesis__container" $ do
                ...
                case state ^. currentCGenesisBlockAddresses of
                    NotAsked  -> emptyView ""
                    Loading   -> emptyView $ translate (I18nL.common <<< I18nL.cLoading) lang'
...
summaryView :: CGenesisBlockSummary -> Language -> P.HTML Action
...
addressesView :: CGenesisBlockAddresses -> State -> P.HTML Action
...
addressView :: CGenesisBlockAddress -> Language -> P.HTML Action
