...
import           Pos.Core (ApplicationName (..), Block, BlockHeader (..), BlockVersion (..),
                     ...
                     GenericBlockHeader (..), HeaderHash, SoftforkRule (..), SoftwareVersion (..),
                     StakeholderId, TxFeePolicy (..), unsafeCoinPortionFromDouble)
import           Pos.Core.Block (BlockHeaderAttributes, BlockSignature (..), MainBlock,
                     MainBlockHeader, MainBlockchain, MainBody (..), MainConsensusData (..),
