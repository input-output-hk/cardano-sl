...
data OutboundQ msg nid buck = ( FormatMsg msg
    ...
    , qConnections     :: TVar (Map nid ConnectionStatus)
    }
...
new qSelf
  ...
  = liftIO $ do
    ...
    qHealth      <- newQHealth
    qConnections <- newTVarIO Map.empty
...
intEnqueue :: forall m msg nid buck a. (MonadIO m, WithLogger m)
           ...
           -> Peers nid
           -> m [Packet msg nid a]
intEnqueue outQ@OutQ{..} msgType msg peers = do
    fmap concat $
    ...
    sendFwdSet warnOnFailure alreadyPicked maxAhead prec (nodeType, alts) = do
      ...
      case mAlt of
        ...
        Just alt -> liftIO $ do
          ...
          atomically $ modifyTVar qConnections $ Map.alter alterConnStatus alt
          let packet = Packet {
                           packetPayload  = msg
                         , packetDestId   = alt
    ...
    sendPacket (EnqPacket p) = do
      ...
      forkThread threadRegistry $ \unmask -> do
          ...
          case ma of
              ...
              Right _  ->
                atomically
                    $ modifyTVar qConnections
                    $ Map.update (const $ Just Connected) (packetDestId p)
...
intFailure OutQ{..} p sendStartTime err = do
    atomically $ modifyTVar qConnections
        $ Map.update (const $ Just ConnectionBroken) (packetDestId p)
      ...
      if    Set.size peers
        ...
        then return (buckets, False)
          ...
          forM_ removed $ \nid -> do
            ...
            atomically $ modifyTVar qConnections $ Map.delete nid
            MQ.removeAllIn (KeyByDest nid) qScheduled
          let newConns = Map.fromList $ map (,NotConnected) $ Set.elems peers
          atomically $ modifyTVar qConnections $ flip Map.union newConns
          return (buckets', True)
