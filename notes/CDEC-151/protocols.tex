...
is the consumer's `read pointer'. This tracks what the producer knows about the
head of the consumer's chain.
...
Figure \ref{chain-diagram-read-pointers} illustrates an example with a producer
and two consumers. In the example, consumer A illustrates a typical case where
the consumer's read pointer is on the producer's chain, while consumer B
illustrates a case involving a fork. We will cover both cases.

\paragraph{Request/Response interaction}
The form of interaction between the consumer and producer follows a simple
request/response pattern. The consumer makes a request to ask for the next
state change and the producer replies with an instruction on how the consumer
should change its state. These update are either to roll forward to a given
block (supplied by value or by reference), or to roll backwards to a given
point on the client's chain.
...
As a special case, if the read pointer is at the head block when the consumer
ask for the next instruction then the producer's response is delayed until
the producer's own chain next changes (by adding a block or rolling back).
Optionally the producer can inform the consumer promptly that it is in this
state and will have to wait for the next update.
...
producer's chain. The state that the producer has in this case ...

% In both cases the producer assumes that the consumer will update its
% own state as instructed and so the producer can update the read pointer for
% this consumer correspondingly.

% In this case when the consumer asks for the next
% change the response is given the next block header; and the producer updates
