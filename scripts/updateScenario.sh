...
if [ ! -f $updatetar ]; then echo "Update tar file at path $updateTar not found, run with --build/-b first" && exit; fi
if [ ! -f binaries_v000/cardano-node ]; then echo "binaries_v000/cardano-node not found run with --build/-b first" && exit; fi
...
if $runNode; then
  ...
  stack exec cardano-launcher -- --node binaries_v000/cardano-node --node-log-config scripts/update-log-config.yaml -n "--update-server"  -n "http://localhost:$serverPort" -n "--update-latest-path" -n "updateDownloaded.tar" -n "--listen" -n "127.0.0.1:3004" -n "--peer" -n "127.0.0.1:3000/a_P8zb6fNP7I2H54FtGuhqxaMDAwMDAwMDAwMDAwMDA=" -n "--flat-distr" -n "(3,100000)" -n "--rebuild-db" -n "--wallet" -n "--web-port" -n 8080 -n "--wallet-port" -n 8090 -n "--wallet-rebuild-db" --updater $updater -u "dir" -u "binaries_v000" --node-timeout 5 --update-archive updateDownloaded.tar $wallet_cli 
