...
import           Pos.Block.Logic             (ClassifyHeaderRes (..),
                                              ...
                                              classifyNewHeader, getHeadersFromToIncl)
...
handleGetHeaders
    :: forall ssc m.
       (WorkMode ssc m)
    => ListenerAction BiP m
handleGetHeaders = ListenerActionConversation $ \__peerId __sendActions conv ->
...
handleBlockHeaders = ListenerActionConversation $
    \peerId sendActions conv -> do
        ...
        whenJust mHeaders $ \(MsgHeaders headers) ->
            handleUnsolicitedHeaders headers peerId sendActions conv
...
handleUnsolicitedHeaders
    ...
    -> m ()
handleUnsolicitedHeaders (header :| []) peerId sendActions conv =
    handleUnsolicitedHeader header peerId sendActions conv
-- TODO: ban node for sending more than one unsolicited header.
handleUnsolicitedHeaders (h:|hs) _ _ _ = do
...
handleUnsolicitedHeader
    ...
    -> ConversationActions (MsgGetHeaders ssc) (MsgHeaders ssc) m
    -> m ()
handleUnsolicitedHeader header peerId sendActions conv = do
    logDebug $ sformat
