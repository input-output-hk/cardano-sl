...
instance SscBi => SecurityWorkersClass SscGodTossing where
    securityWorkers = Tagged [ onNewSlot True checkForReceivedBlocksWorker
...
checkForReceivedBlocksWorker :: WorkMode ssc m => SlotId -> Worker m
checkForReceivedBlocksWorker slotId = worker $ \sendActions -> do
...
checkForIgnoredCommitmentsWorker :: forall m. WorkMode SscGodTossing m => Worker m
checkForIgnoredCommitmentsWorker = worker $ \sA -> do
    epochIdx <- atomically (newTVar 0)
    _ <- runReaderT (onNewSlot True (const . checkForIgnoredCommitmentsWorkerImpl) sA) epochIdx
    return ()
