...
import           Pos.State.Storage.Block                   (BlockStorage,
                                                            ...
                                                            blkCreateNewBlock,
                                                            blkProcessBlock, blkRollback,
                                                            blkSetHead, getBestChain,
                                                            getBlock, getHeadBlock,
                                                            getLeaders, getSlotDepth,
                                                            mayBlockBeUseful,
                                                            mkBlockStorage)
import           Pos.State.Storage.Stats                   (HasStatsData (statsData),
                                                            StatsData, getStatRecords,
                                                            newStatRecord)
import           Pos.State.Storage.Tx                      (HasTxStorage (txStorage),
                                                            TxStorage, getLocalTxs,
                                                            getUtxoByDepth, processTx,
...
import           Pos.Types                                 (Block, EpochIndex,
                                                            GenesisBlock, MainBlock,
                                                            SlotId (..), SlotLeaders,
                                                            Utxo, blockMpc, blockSlot,
                                                            blockTxs, epochIndexL,
                                                            flattenSlotId, gbHeader,
                                                            headerHashG, unflattenSlotId,
                                                            verifyTxAlone)
...
setSecret = ssSetSecret @GtSecretStorage

getSecret :: SecQuery (Maybe GtSecret)
getSecret = ssGetSecret @GtSecretStorage

prepareSecretToNewSlot :: SlotId -> SecUpdate ()
prepareSecretToNewSlot = ssPrepareToNewSlot @GtSecretStorage
