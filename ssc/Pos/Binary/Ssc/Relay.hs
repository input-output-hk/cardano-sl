...
instance Bi (DataMsg T.MCOpening) where
    encode (DataMsg (T.MCOpening sId opening)) =
        encodeListLen 2 <> encode sId <> encode opening
    decode = do
        enforceSize "DataMsg T.MCOpening" 2
        DataMsg <$> (T.MCOpening <$> decode <*> decode)

instance Bi (DataMsg T.MCShares) where
    encode (DataMsg (T.MCShares sId innerMap)) =
        encodeListLen 2 <> encode sId <> encode innerMap
    decode = do
