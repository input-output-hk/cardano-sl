...
sscWorkers
  ...
  -> [Diffusion m -> m ()]
sscWorkers pm namedLogTrace = 
    [ onNewSlotSsc pm namedLogTrace
...
onNewSlotSsc
    ...
    -> Diffusion m
    -> m ()
onNewSlotSsc pm namedLogTrace = \diffusion -> onNewSlotNoLogging defaultOnNewSlotParams $ \slotId ->
    recoveryCommGuard logTrace "onNewSlot worker in SSC" $ do
        ...
        whenM (shouldParticipate logTrace $ siEpoch slotId) $ do
            ...
            checkNSendOurCert pm logTrace (sendSscCert diffusion)
            onNewSlotCommitment pm logTrace slotId (sendSscCommitment diffusion)
            onNewSlotOpening pm logTrace (sbSendOpening behavior) slotId (sendSscOpening diffusion)
            onNewSlotShares pm logTrace (sbSendShares behavior) slotId (sendSscShares diffusion)
    where   
...
checkNSendOurCert logTrace sendCert = do
    ...
    let sendCertDo resend slot = do
            ...
            sscProcessOurMessage logTrace (sscProcessCertificate pm logTrace ourVssCertificate)
...
onNewSlotCommitment
    ...
    -> (SignedCommitment -> m ())
    -> m ()
onNewSlotCommitment pm logTrace slotId@SlotId {..} sendCommitment
  ...
  where
    onNewSlotCommDo = do
        ...
        generated <- generateAndSetNewSecret pm logTrace ourSk slotId
        case generated of
    ...
    sendOurCommitment comm = do
        sscProcessOurMessage logTrace (sscProcessCommitment pm logTrace comm)
...
onNewSlotOpening
    ...
    -> (Opening -> m ())
    -> m ()
onNewSlotOpening pm logTrace params SlotId {..} sendOpening
  ...
  where
    ...
    sendOpeningDo ourId open = do
        ...
        whenJust mbOpen' $ \open' -> do
            sscProcessOurMessage logTrace (sscProcessOpening pm logTrace ourId open')
...
onNewSlotShares
    ...
    -> (InnerSharesMap -> m ())
    -> m ()
onNewSlotShares pm logTrace params SlotId {..} sendShares = do
    ourId <- getOurStakeholderId
  ...
  where
    sendSharesDo ourId shares = do
        ...
        unless (HM.null shares') $ do
            ...
            sscProcessOurMessage logTrace (sscProcessShares pm logTrace ourId lShares)
