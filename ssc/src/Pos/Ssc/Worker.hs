...
sscWorkers
  ...
  => ProtocolMagic -> ProtocolConstants -> [Diffusion m -> m ()]
sscWorkers pm pc =
    [ onNewSlotSsc pm pc
...
onNewSlotSsc
    ...
    -> Diffusion m
    -> m ()
onNewSlotSsc pm pc diffusion = onNewSlot (pcEpochSlots pc) defaultOnNewSlotParams $ \slotId ->
    recoveryCommGuard (pcBlkSecurityParam pc) "onNewSlot worker in SSC" $ do
        ...
        whenM (shouldParticipate $ siEpoch slotId) $ do
            ...
            onNewSlotCommitment pm pc slotId (sendSscCommitment diffusion)
            onNewSlotOpening pm pc (sbSendOpening behavior) slotId (sendSscOpening diffusion)
            onNewSlotShares pm pc (sbSendShares behavior) slotId (sendSscShares diffusion)
