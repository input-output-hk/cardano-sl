...
Cardano does not implement the entire Wallet Spec, for instance *expected UTxO* and associated properties like *minimum balance* are not currently supported (and are not represented in the DSL model either). These cases will be clearly highlighted in this document.
...
During every wallet operation, e.g. `applyBlock`, a new checkpoint is added. Conversely, on `rollback`, we can rely on previous checkpoint data.
...
* `applyBlock` will be applide to *every* account in *every* local HD wallet, similary `switchToFork` is applied to all local accounts
...
The core implementations of Cardano wallet updates (used by the AcidState layer) are found in

At this layer we have pure functions that act on single account checkpoints, e.g.
...
* for `updatePending`, see Sec 3 - Basic Model
* for `updateBlockMeta`, see Sec 9 - Tracking Metadata
...
`newForeign` caters for transfering funds from another wallet to the local wallet. The invariant is the inverse of `newPending`, in this case _none_ of the inputs must belong to the addressses of the receiving wallet.
...
Cardano ensures that empty checkpoints (for _all_ wallet accounts) are created for every new block the wallet is notifed of. See [AcidState.applyBlock](https://github.com/input-output-hk/cardano-sl/blob/6659d8501c727714a7861ad2e527a337e0a11b86/wallet-new/src/Cardano/Wallet/Kernel/DB/AcidState.hs#L223)

See [rollback trivial case](https://github.com/input-output-hk/cardano-sl/blob/6659d8501c727714a7861ad2e527a337e0a11b86/wallet-new/src/Cardano/Wallet/Kernel/DB/Spec/Update.hs#L216) for the Cardano implementation of a "useful optimisation opportunity: we can simply leave all the empty checkpoints as implicit".
...
* Spec Sec 6.5 "Switching to a fork"
...
Note: Cardano wallet state is expressed as an `AcidState` database while transaction metadata is stored separately in a `sqlite` database.
...
##### LocalBlockMeta and applyBlockPartial
...
`LocalBlockMeta` becomes relevant during wallet restoration, during which we have incomplete context of the blockchain. In this case we use `applyBlockPartial` that builds checkpoints with "local" (possibly incomplete) instead of complete block metadata.
...
(see `applyBlock` in Sec 5 - Prefiltering)
...
Block and Address metadata is constructed during the [prefiltering stage](https://github.com/input-output-hk/cardano-sl/blob/6659d8501c727714a7861ad2e527a337e0a11b86/wallet-new/src/Cardano/Wallet/Kernel/PrefilterTx.hs#L304-L345).
...
The resubmission function `tickFunction` triggers the `tick` and cancels any pending transactions returned:
