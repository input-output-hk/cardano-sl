...
is summarized more formally in Figure~\ref{fig:input_selection_sig}.
...
this process, prioritizing some goals over others.
...
optimally. Note that minimizing transaction fees over time does not necessarily
...
As an example of how different users might prioritize different goals,
...
not a goal of the policy. Exchanges may occassionally need to make very large
payments.
...
Although our formalization of UTxO style accounting is based on that of
\cite{utxo_accounting}, we diverge slightly from that formalization and do
...
\mathsf{totalin}_\mathit{utxo} & ( & \mathit{inputs}, ~ \_  & ) & = \mathsf{balance} ~ (\mathit{inputs} \restrictdom \mathit{utxo}) \\
\mathsf{totalout}              & ( & \_, ~ \mathit{outputs} & ) & = \sum_{(\_, c) \in \mathit{outputs}} c
\end{IEEEeqnarray*}
%
...
% isUsed
%   an address a is _used_ iff
%     1. it is owned by the wallet, and
%     2. there is confirmed transaction which outputs to this address
%   (i.e., pending transactions are _not_ included here)
%
% isChange (our definition):
%   an address a is considered a change address iff
%     1. ours a
%     2. there are no _other_ confirmed transactions that output to this address
%     3. there exists a confirmed transaction which
%          3a. outputs to that address
%          3b. there is at least one output that does _not_ belong to the wallet
%          3c. all of the transaction's inputs belong to the wallet
%   (redemption?)
%
