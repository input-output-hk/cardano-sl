...
The term `self organization' refers to the emergence of complex behaviour
(typically in biological systems) from simple rules and random fluctuations.
In this section we will see how we can take advantage of self organization
to design a simple yet effective coin selection algorithm.

\emph{Coin selection} is the process of selecting unspent outputs in a user's
wallet to satisfy a particular payment request. As Jameson Lopp points out in
his blog post ``The Challenges of Optimizing Unspent Output Selection''
\citep{lopp:challenges}, coin selection is thorny problem. Moreover, there is a
surprising lack of academic publications on the topic; indeed, the only
scientific study of coin selection appears to be Mark Erhardt's masters thesis
``An Evaluation of Coin Selection Strategies'' \citep{Erhardt:thesis}.
...
straight-forward to implement, and through studing simulations of the algorithm
under various conditions we will see that it is very effective. Erhardt's random
algorithm provides our jumping off point, from which we will make various
improvements.
...
There are various things to see in this animation, but for now we want to focus
...
once it kicks in. If you watch the inputs-per-transaction histogram, you can see
...
We will restate Erhardt's observation as our first self organization principle:
%
\begin{quote}
...
of inputs. We can take advantage of self organization again:
%
\begin{quote}
...
Fortunately, self organization comes to the rescue again. We can set an upper
...
self organization principles we isolated really mean that order will emerge from
...
emphasize in that section is that the algorithm we simulated there is actually
...
\caption{\label{fig:inputselection:MoneyPot_randomOn}
  \texttt{Random-Improve}, using the MoneyPot data set. There is a rougly 2:1
...
deposit:payment ratio of roughly 30:1. Under these circumstances, of course,
coin selection alone cannot keep the UTxO size small
(Figure~\ref{fig:inputselection:CardanoExchange_randomOn}).
...
\caption{\label{fig:inputselection:CardanoExchange_randomOn}
  ...
  There is a rougly 30:1 deposit:payment ratio. Values have been scaled.
  Log scale on the x-axis.
}
\end{figure}

The UTxO here also contains some truly enormous outputs, which is pushing the
change:payment ratio up, although as more withdrawals are being made the
algorithm is able to push it down. Coin selection alone cannot deal with
a situation like this (and indeed, exchanges have additional infrastructure in
place that alleviates this problem).
...
selection algorithm. Branch and Bound in essense is a limited backtracking
...
relying on self organization principles, the first of which was stated by
...
deposits, we could occassionally issue a ``reorganization transaction'' that
...
need for dependent transactions). Such reorganization is largely orthogonal to
