...
\cref{lem:wallet_later_created} gives us a very useful optimization opportunity:
...
empty, and the block metadata only records informatin about transactions that
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
  ...
  \path[every node/.style={font=\sffamily\scriptsize,
    ...
    ;
\end{tikzpicture}
\caption{\label{fig:transaction_state}Transaction state transitions (outgoing, \emph{left}, and incoming, \emph{right}). We will come back to the marked transistions ($\dagger$) in Section~\ref{sec:submission_interface}.}
\end{figure}
