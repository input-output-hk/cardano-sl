...
pay activeWallet genChangeAddr mkSigner opts accountId payees = do
        retrying retryPolicy shouldRetry $ \rs -> do
            tx <- newTransaction activeWallet genChangeAddr mkSigner opts accountId payees
            case tx of
...
newTransaction ActiveWallet{..} genChangeAddr mkSigner options accountId payees = do
    ...
    let mkTx = mkStdTx walletProtocolMagic mkSigner
...
estimateFees activeWallet@ActiveWallet{..} genChangeAddr mkSigner options accountId payees = do
    snapshot         <- getWalletSnapshot walletPassive
    let originalUtxo = accountUtxo snapshot accountId
    res <- newTransaction activeWallet genChangeAddr mkSigner options accountId payees
    case res of
