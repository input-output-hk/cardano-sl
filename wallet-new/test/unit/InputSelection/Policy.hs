...
newtype InputPolicyT h a m x = InputPolicyT {
      unInputPolicyT :: StrictStateT (InputPolicyState h a) (ExceptT [InputSelectionFailure a] m) x
    }
  deriving ( Functor
           , Applicative
           , Monad
...
runInputPolicyT estimateFee expenseRegulation originalUtxo originalOutputs policyT = do
     mx <- runExceptT (runStrictStateT (unInputPolicyT (policyT originalOutputs)) initSt)
     case mx of
       Left errs ->
         return $ Left errs
