...
import           Cardano.Wallet.Kernel.DB.Spec (Pending (..), emptyPending, genPending,
                                                pendingTransactions, singletonPending)
...
import           Test.QuickCheck (Gen, Property, arbitrary, conjoin, forAll, vectorOf, (===), (==>))
...
afterTickProperty :: (a,WalletSubmission m)
                  -> ((a, WalletSubmission m) -> Property)
                  -> Property
afterTickProperty result fn = fn result
...
dependentTransactions :: Gen [Core.TxAux]
...
genPureWalletSubmission = STB <$> genWalletSubmission constantResubmit

genPurePair :: Gen (ShowThroughBuild (WalletSubmission Identity, Pending))
genPurePair = do
    pair <- (,) <$> (fmap unSTB genPureWalletSubmission)
...
includeEvent :: ScheduleEvents -> Core.TxAux -> Property
includeEvent se tx =
    failIf "doesn't include event"
              (\t s -> hash (Core.taTx t) `List.elem` toTxIds (s ^. seToSend)) tx se

includeEvents :: ScheduleEvents -> [Core.TxAux] -> Property
includeEvents se txs = failIf "not includes all of" checkEvent se txs
    where
        checkEvent :: ScheduleEvents -> [Core.TxAux] -> Bool
        checkEvent (ScheduleEvents toSend toConfirm) txs =
          all (\t -> hash (Core.taTx t) `List.elem` toTxIds toSend) txs

mustNotIncludeEvents :: ScheduleEvents -> [Core.TxAux] -> Property
mustNotIncludeEvents se txs = failIf "does include one of" checkEvent se txs
    where
        checkEvent :: ScheduleEvents -> [Core.TxAux] -> Bool
        checkEvent (ScheduleEvents toSend toConfirm) txs =
          all (\t -> not $ hash (Core.taTx t) `List.elem` toTxIds toSend) txs
    ...
    describe "Test wallet submission layer" $ do
      ...
      it "constantRetry works predictably" $ do
           ...
           conjoin [
              ...
              , policy (SubmissionCount 6) (Slot 6) === CheckConfirmedIn (Slot 7)
              , policy (SubmissionCount 9) (Slot 0) === CheckConfirmedIn (Slot 1)
              ]

      it "limit retries correctly" $ do
          forAll genPurePair $ \(unSTB -> (ws, pending)) ->
              let ws' = (addPending pending ws) & wsResubmissionFunction .~ giveUpAfter 5
                  updateFn (d, newWs) _ = bimap (S.union d) identity (tick' newWs)
                  (dropped, _) = List.foldl' updateFn (S.empty, ws') ([1..6] :: [Int])
                  in failIf "not subset of" S.isSubsetOf (toTxIdSet pending) dropped
      ...
      describe "tickSlot" $ do
          ...
          it "Given D->C->B->A all in the same slot, they are all sent" $ do
              let generator = do txs <- dependentTransactions
                                 ws  <- addPending (pendingFromTxs txs) . unSTB <$> genPureWalletSubmission
                                 return $ STB (ws, txs)
              forAll generator $ \(unSTB -> (submission, txs)) ->
                  ...
                  in case result of
                          ...
                          Right (toSend, _, _) -> conjoin [
                                scheduledEvents `includeEvents` txs
          ...
          it "Given D->C->B->A, if C,B,A are in the future, D is not sent this slot" $ do
              let generator = do [b,c,a,d] <- dependentTransactions
                                 ws  <- addPending (pendingFromTxs [a,b,c]) . unSTB <$> genPureWalletSubmission
                                 return $ STB (addPending (pendingFromTxs [d]) (snd $ tick' ws), d)
              forAll generator $ \(unSTB -> (submission, d)) ->
                  ...
                  in case result of
                          Left err -> property $ exception "tickSlot found a loop" (toException err)
                          Right (toSend, _, _) -> conjoin [
                                scheduledEvents `includeEvent` d
                              , failIf "is subset of"
          ...
          it "Given D->C->B->A, can send [A,B] now, [D,C] in the future" $ do
              ...
              forAll generator $ \(unSTB -> (submission1, [a,b,c,d])) ->
                  let slot1     = submission1 ^. getCurrentSlot
                      ...
                      modifyPending = addPending (pendingFromTxs [c])
                                    . remPending (pendingFromTxs [a,b])
      ...
      it "won't drop transactions which has been successfully transmitted" $ do
          forAll genPurePair $ \(unSTB -> (submission, pending)) ->
              pending /= emptyPending ==>
                  ...
                  in afterTickProperty dropped (\(p, _) -> property $ not $ p `S.isSubsetOf` (toTxIdSet pending))
      ...
      it "can drop transactions which exceeded the resubmission count" $ do
          forAll genPurePair $ \(unSTB -> (submission, pending)) ->
              ...
              in afterTickProperty result (\(p, _) -> property $ p `S.isSubsetOf` (toTxIdSet pending))
