...
initActors CardanoContext{..} = Actors{..}
  where
    ...
    mkRich RichSecrets{..} = (regKpPub richKey, Rich {..})
      where
        richKey :: RegularKeyPair
        richKey = regularKeyPair rsPrimaryKey

        richAddr :: Address
        richAddr = makePubKeyAddressBoot (toPublic rsPrimaryKey)

    mkPoor :: EncryptedSecretKey -> (PublicKey, Poor)
    mkPoor poorSec = (encKpPub poorKey, Poor {..})
      where
        poorKey :: EncKeyPair
...
initAddrMap Actors{..} = AddrMap{
  ...
  where
    ...
    mkMap = concat [
          map       mkRich $ zip [0..] (Map.elems actorsRich)
        , concatMap mkPoor $ zip [0..] (Map.elems actorsPoor)
        , map       mkAvvm $ zip [0..] (Map.elems actorsAvvm)
        ]
    ...
    mkPoor (actorIx, Poor{..}) = map poorRawAddr $ zip [0..] poorAddrs
      where
