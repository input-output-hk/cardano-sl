...
utxoAfter l = Utxo $ allOutputs `withoutKeys` allSpent
  where
    ...
    outputsOf t = map (\(i, o) -> (Input t i, o)) (zip [0..] (trOuts t))
...
example1 :: forall a. a -> a -> a -> [Transaction a]
example1 a1 a2 a3 = [t5, t4, t3, t2, t1]
  where
    t1, t2, t3, t4, t5 :: Transaction a
    t1 = Transaction []           [ Output  AddrTreasury     1000 ]
    t2 = Transaction [Input t1 0] [ Output (AddrOrdinary a1) 1000 ]
    t3 = Transaction [Input t2 0] [ Output (AddrOrdinary a2)  800
                                  , Output (AddrOrdinary a1)  200 ]
    t4 = Transaction [Input t3 1] [ Output (AddrOrdinary a3)  199
                                  , Output  AddrTreasury        1 ]
    t5 = Transaction [Input t4 1] [ Output (AddrOrdinary a2)    1 ]
