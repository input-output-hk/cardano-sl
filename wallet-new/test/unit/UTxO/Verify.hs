...
mapErrors :: (e -> e') -> Verify e a -> Verify e' a
mapErrors f (Verify ma) = Verify $ mapStateT' (withExceptT f) ma
...
verifyBlocksPrefix tip curSlot leaders lastSlots blocks = do
    let actualTip :: HeaderHash
        actualTip = blocks ^. _Wrapped . _neHead . prevBlockL
    when (tip /= actualTip) $
        throwError $ VerifyBlocksError
          (sformat ("the first block isn't based on the tip (expected "
                   % build
                   % ", got "
                   % build)
                   tip
    ...
    slogUndos <- mapErrors VerifyBlocksError $
                   slogVerifyBlocks curSlot leaders lastSlots blocks
    txUndo    <- mapErrors (VerifyBlocksError . pretty) $
                   tgsVerifyBlocks $ map toTxpBlock blocks

    -- Since we skip delegation and US checks, we construct dummy undo values
    -- for those. The wallet doesn't care anyway. (TODO: Is that true?)
    let pskUndo :: OldestFirst NE DlgUndo
        pskUndo = map (const (DlgUndo [] HS.empty)) slogUndos

        usUndos :: OldestFirst NE USUndo
        usUndos = map (const def) slogUndos

    -- Eventually we do a sanity check just in case and return the result.
    -- NOTE: This check was originally @length txUndo /= length pskUndo@.
...
slogVerifyBlocks curSlot leaders lastSlots blocks = do
    ...
    -- At this point verification is done and we just construct some pure
    -- values. This is unchanged from the original:
