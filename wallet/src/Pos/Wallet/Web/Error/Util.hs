...
catchEndpointErrors
    ...
    => TraceNamed m
    -> m a
    -> m (Either WalletError a)
catchEndpointErrors logTrace action = catchOtherError $ tryWalletError action
  where
    tryWalletError  = tryJust $ \e -> (e ^? _RequestError) $> e
    catchOtherError = flip catches
        [ Handler $ throwM @_ @ServantErr
        , Handler $ \(SomeException e) -> do
            logError (appendName logName logTrace) $
                sformat ("Uncaught error in wallet method: "%shown) e
