...
getWalletIncludeUnready nm ws mps includeUnready cAddr = do
    ...
    accMod     <- txMempoolToModifier ws mps . (eskToWalletDecrCredentials nm) $ key
...
newAccountIncludeUnready nm includeUnready addGenSeed passphrase CAccountInit {..} = do
    ...
    accMod <- txMempoolToModifier ws mempool . (eskToWalletDecrCredentials nm) =<< findKey nm caInitWId
    -- check wallet exists
